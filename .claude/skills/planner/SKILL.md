...
##### Priorización por riesgo (P0-P3)

El ceremony level dice cuánto *proceso*; el risk score dice cuánto *test*. Son ortogonales. Puntuá cada FILE-ID o área funcional:

```
Business Impact (B):    1=cosmético  2=UX degradada  3=problema de datos  4=revenue  5=legal/safety
Defect Probability (P): 1=muy estable  2=historia menor  3=algunos bugs  4=frecuente  5=conocido frágil

Risk Score = B × P

≥16 → P0  (release blocker si se rompe — coverage profundo + charter exploratorio)
12-15 → P1  (testear sí o sí antes del release)
6-11  → P2  (suite de regresión)
1-5   → P3  (exploratorio, baja frecuencia)
```

Agregá la columna `Prioridad` a la Coverage Matrix. Los FILE-IDs P0/P1 marcan dónde concentrar TEST-IDs; los P3 pueden quedar en exploratorio. @qa usa esto en el sign-off para justificar profundidad de coverage por área.

**ROI de automatización** (cuándo automatizar vs dejar manual): `(Frecuencia de ejecución × Risk Score) / Costo de implementación`. ROI alto → automatizar primero. ROI bajo → manual o diferir.
